...
# Virtual contracts stay explicit so a toolchain import change requires review.
# The installer smoke below this audit executes both the source and installed release binary.
$AllowedApiSetContracts = @(
...
foreach ($ImportedDll in $ImportedDlls) {
    ...
    if ($ImportedDll -match $ApiSetPattern) {
        if ($AllowedApiSetContracts -notcontains $ImportedDll) {
            throw "Windows release binary imports an unreviewed API-set contract: $ImportedDll"
        }
        Write-Output "reviewed API-set import: $ImportedDll"
        continue
    }

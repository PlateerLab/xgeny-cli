...
workspace mode는 `list-directory`, `stat`, `search-text`, `read-text`, `write-atomic`, `apply-patch`를
제공한다. 명시적 executable catalog와 별도 실행 승인은
[Process execute adapter](process-execute-adapter.md)를 따른다.

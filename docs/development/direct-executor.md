...
ADR-0014는 Receipt-bound terminal gate를 dependency release 조건으로 사용하는 Tracked/Persistent WorkGraph와 restart frontier를 추가했고, ADR-0015는 actionable `Planned` Step을 만드는 durable plan input과 provider-neutral bounded AgentLoop 계약을 추가했다. ADR-0017은 이 typed port에 첫 OpenAI-compatible 모델 provider를 연결했다. ADR-0018은 WorkGraph `ReadOnly`, bounded Artifact Receipt와 아직 사용자 binary command에 노출하지 않은 generic CLI driver 기반을 추가했다. 후속은 schema 7 typed output/completion durability, 제품용 read-only filesystem confinement, public composition 순으로 분리한다. 제품 adapter는 참조 adapter의 단순 확장이 아니며 root directory capability와 symlink/junction/TOCTOU 규칙을 별도 gate로 통과해야 한다. Adapter definite failure·unknown Receipt, credential resolver와 process adapter도 각각 분리한다.

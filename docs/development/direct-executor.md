...
ADR-0014는 Receipt-bound terminal gate를 dependency release 조건으로 사용하는 Tracked/Persistent WorkGraph와 restart frontier를 추가했고, ADR-0015는 actionable `Planned` Step을 만드는 durable plan input과 provider-neutral bounded AgentLoop 계약을 추가했다. ADR-0017은 이 typed port에 첫 OpenAI-compatible 모델 provider를 연결했다. ADR-0018은 WorkGraph `ReadOnly`, bounded Artifact Receipt와 아직 사용자 binary command에 노출하지 않은 generic CLI driver 기반을 추가했다. ADR-0019는 exact output schema를 먼저 검증한 bounded `ToolOutputRecord`, event-anchored schema 7 sidecar와 restart-safe verification snapshot을 추가했다. ADR-0020은 same-generation PlanningContext v2로 completed output의 다음 model turn 전달을 닫았고, ADR-0030은 v3에서 plan/Receipt journal chronology까지 보존한다. ADR-0021은 `CompletionOutputRecord`와 schema 8로 exact 최종 summary의 process-restart 복원을 닫았다. 후속은 제품용 read-only filesystem confinement와 public composition 순으로 분리한다. 제품 adapter는 참조 adapter의 단순 확장이 아니며 root directory capability와 symlink/junction/TOCTOU 규칙을 별도 gate로 통과해야 한다. Adapter definite failure·unknown Receipt, credential resolver와 process adapter도 각각 분리한다.

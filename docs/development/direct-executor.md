...
ADR-0014는 Receipt-bound terminal gate를 dependency release 조건으로 사용하는 Tracked/Persistent WorkGraph와 restart frontier를 추가했고, ADR-0015는 actionable `Planned` Step을 만드는 durable plan input과 provider-neutral bounded AgentLoop 계약을 추가했다. ADR-0017은 이 typed port에 첫 OpenAI-compatible 모델 provider를 연결했다. 다음 실행 slice는 사용자용 CLI driver와 제품용 read-only filesystem end-to-end를 닫는 일이다. 제품 adapter는 참조 adapter의 단순 확장이 아니다. WorkGraph `ReadOnly`, bounded typed output와 Artifact, root directory capability와 symlink/junction/TOCTOU 규칙을 별도 ADR·gate에서 먼저 확정한다. Adapter definite failure·unknown Receipt, credential resolver와 process adapter도 각각 분리한다.

...
`xgeny` binary에는 `run` 명령이 없다. 실제 파일을 여는 제품 adapter, 다음 planning turn의 output context와 completion 원문 저장도 없다. 따라서 durable typed output 기반만을 “Claude Code/Codex와 같은 파일 읽기”, “모델이 파일 내용을 이어서 사용함” 또는 “실제 workspace sandbox 검증 완료”로 설명하면 안 된다.
...
ReadOnly/driver 기반은 [ADR-0018](../adr/0018-read-only-artifact-and-cli-driver-foundation.md), durable output과 다음 제품 slice의 보안 gate는 [ADR-0019](../adr/0019-durable-tool-output-and-schema-7.md)을 따른다.
